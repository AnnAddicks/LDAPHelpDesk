...
public class ModifyPasswordRequest implements ExtendedRequest, Serializable {
   ...
   * 
   * @param dn
   *          the dn whose password is to change
   * @param password
   *          the new password
   * @exception NullPointerException
   *              if dn or password is null
   * @exception SizeLimitExceededException
   *              when the dn or password is too long
   */
  public ModifyPasswordRequest(final String dn, final String password) throws NullPointerException,
    ...
    if (totallen > 0xFF) {
      throw new SizeLimitExceededException("the lengh of the dn + the lengh of the password cannot"
  ...
  /**
   * Creates the extended response. With OpenLDAP, the extended operation for
   * Password modification doesn't create a response so we just return null
   * here.
   * 
   * @param id
   *          the OID of the response
   * @param berValue
   *          the BER encoded value of the response
   * @param offset
   *          the offset
   * @param length
   *          the length of the response
   * @return returns null as the modify password operation doesn't generate a
   *         response.
   */
  @Override
  public ExtendedResponse createExtendedResponse(final String id, final byte[] berValue,
